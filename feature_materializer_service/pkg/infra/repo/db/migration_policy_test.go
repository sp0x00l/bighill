...
var _ = Describe("feature materializer migration policy", func() {
	It("keeps tenant-scoped tables fail-closed", func() {
		...
		Expect(content).To(ContainSubstring("CREATE TYPE assertion_status_enum AS ENUM ('candidate', 'admitted', 'rejected', 'revoked')"))
		Expect(content).To(ContainSubstring("assertion_status assertion_status_enum NOT NULL DEFAULT 'admitted'"))
		Expect(content).To(ContainSubstring("index_embedding_records_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_nodes_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_node_aliases_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_node_embeddings_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_edges_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_node_chunks_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_communities_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_community_members_assertion_status"))
		Expect(content).To(ContainSubstring("index_graph_community_reports_assertion_status"))
