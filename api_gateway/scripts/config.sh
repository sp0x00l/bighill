...
if [ "$ENVIRONMENT" = "local-dev" ] || [ "$ENVIRONMENT" = "cicd" ]; then
    ...
    export DATA_INGESTION_SERVICE_HTTP_HOST="$API_GATEWAY_SERVICE_HOST"
    ...
    export DATA_INGESTION_SERVICE_HTTP_HOST=data-ingestion-service
    export PROFILE_SERVICE_HTTP_HOST=profile-service
fi

export DATA_REGISTRY_SERVICE_HTTP_PORT=8081
export DATA_INGESTION_SERVICE_HTTP_PORT=8086
export PROFILE_SERVICE_HTTP_PORT=8082
export API_HTTP_CLIENT_TIMEOUT_SECONDS=10
