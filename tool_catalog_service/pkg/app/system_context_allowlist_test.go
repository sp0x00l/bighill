...
var _ = Describe("tool catalog system context allowlist", func() {
	It("allows only audited production call sites", func() {
		...
		allowed := map[string]int{
			"pkg/app/tool_catalog_usecase.go": 1,
		}

		violations := toolCatalogSystemContextViolations(serviceRoot, allowed)
		Expect(violations).To(BeEmpty(), strings.Join(violations, "\n"))
	})
})

func toolCatalogSystemContextViolations(serviceRoot string, allowed map[string]int) []string {
	violations := []string{}
