...
discover_profile_oauth_credentials() {
  ...
  if [ -n "${TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID:-}" ] || [ -n "${TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET:-}" ] || [ -n "${TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID:-}" ] || [ -n "${TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET:-}" ]; then
    if [ -z "${TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID:-}" ] || [ -z "${TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET:-}" ] || [ -z "${TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID:-}" ] || [ -z "${TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET:-}" ]; then
      echo "Error: TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID, TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET, TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID, and TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET must all be set together."
      exit 1
    fi

    echo "Syncing profile OAuth credentials from config to AWS Secrets Manager..."
    SECRET_JSON=$(jq -n \
      --arg google_client_id "$TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID" \
      --arg google_client_secret "$TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET" \
      --arg discord_client_id "$TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID" \
      --arg discord_client_secret "$TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET" \
      '{
  ...
  TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID=$(echo "$SECRET_JSON" | jq -r '.google_client_id // empty')
  TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET=$(echo "$SECRET_JSON" | jq -r '.google_client_secret // empty')
  TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID=$(echo "$SECRET_JSON" | jq -r '.discord_client_id // empty')
  TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET=$(echo "$SECRET_JSON" | jq -r '.discord_client_secret // empty')
...
create_k8s_profile_oauth_secret() {
  ...
  echo "Creating Kubernetes secret '${SECRET_NAME}'..."
  kubectl -n "$NAMESPACE" create secret generic "$SECRET_NAME" \
    --from-literal=google_client_id="$TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID" \
    --from-literal=google_client_secret="$TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET" \
    --from-literal=discord_client_id="$TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID" \
    --from-literal=discord_client_secret="$TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET" \
    --dry-run=client -o yaml | kubectl apply -f -
}
