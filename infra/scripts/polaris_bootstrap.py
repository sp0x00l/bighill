...
def wait_for_polaris(token: str) -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            return
        except Exception as err:  # noqa: BLE001 - bootstrap should keep polling transient startup failures.
            last_error = err
            time.sleep(2)
...
def ensure_catalog_role(token: str) -> None:
    ...
    request(
        "POST",
        f"/api/management/v1/catalogs/{quote(CATALOG)}/catalog-roles",
        token,
...
def assign_principal_role(token: str) -> None:
    request(
        "PUT",
        f"/api/management/v1/principals/{quote(SERVICE_PRINCIPAL)}/principal-roles",
        token,
        {"principalRole": {"name": PRINCIPAL_ROLE}},
        accept=(200, 201, 409),
    )
...
def assign_catalog_role(token: str) -> None:
    request(
        "PUT",
        f"/api/management/v1/principal-roles/{quote(PRINCIPAL_ROLE)}/catalog-roles/{quote(CATALOG)}",
        token,
        {"catalogRole": {"name": CATALOG_ROLE}},
        accept=(200, 201, 409),
    )
...
if __name__ == "__main__":
    try:
        main()
    except Exception as exc:  # noqa: BLE001 - script entrypoint should print a concise fatal.
