...
- The Go gateway can run this executable with `DATA_STREAM_QUERY_ENGINE_MODE=datafusion`, then stream the returned Arrow records over Flight.
- Local-dev defaults to `DATA_STREAM_QUERY_ENGINE_MODE=registry` so the stream service resolves registered datasources through the data registry. Use `DATA_STREAM_QUERY_ENGINE_MODE=datafusion` when you want the Rust query engine path.
