...
var _ = Describe("ingestion migration policy", func() {
	It("keeps tenant-scoped tables fail-closed", func() {
		...
		Expect(content).To(ContainSubstring("ALTER TABLE bighill_ingestion_db.tenants FORCE ROW LEVEL SECURITY"))
		Expect(content).To(ContainSubstring("NULLIF(current_setting('app.current_user_id', true), '')::uuid = id"))
		for _, table := range []string{"datasets", "upload_sessions"} {
			Expect(content).To(ContainSubstring("ALTER TABLE bighill_ingestion_db." + table + " FORCE ROW LEVEL SECURITY"))
