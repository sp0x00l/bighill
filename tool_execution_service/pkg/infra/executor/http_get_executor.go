...
const (
	toolArgsSchemaResource       = "https://bighill.ai/schemas/tool_args.schema.json"
	metadataHost                 = "metadata.google.internal"
	httpToolRequestFailedCode    = "http_tool_request_failed"
	httpToolRequestFailedMessage = "http tool request failed"
)
...
func NewHTTPGetExecutor(client HTTPClient, validate *validator.Validate) *HTTPGetExecutor {
	log.Trace("NewHTTPGetExecutor")

	if validate == nil {
		log.Fatal("http get payload validator is required")
	}
...
func (e *HTTPGetExecutor) Execute(ctx context.Context, tool *model.ToolDefinition, command model.InvokeToolCommand, policy model.PolicySet) (*model.ToolInvocationResult, error) {
	...
	args, err := parseHTTPGetRequestPayload(command.ArgumentsJSON, e.validator)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		...
		return &model.ToolInvocationResult{
			...
			ErrorMessage:          httpToolRequestFailedMessage,
			ErrorType:             errorType,
...
func httpErrorCode(status int) string {
	...
	if status >= http.StatusBadRequest {
		return httpToolRequestFailedCode
	}
	return ""
}
