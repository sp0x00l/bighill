...
func TestBoundaryPolicyResolver(t *testing.T) {
	...
	RunSpecs(t, "Tool execution boundary policy suite")
}
...
var _ = Describe("BoundaryPolicyResolver", func() {
	...
	It("prefers catalog-projected limits and credential refs over pinned defaults", func() {
		resolver := NewBoundaryPolicyResolver(BoundaryPolicyConfig{
			HTTPTimeout:            3 * time.Second,
			HTTPMaxResponseBytes:   4096,
			PinnedMCPCredentialRef: "PINNED_MCP_TOKEN",
		})
		tool := &model.ToolDefinition{
			Name:             "partner_tool",
			ExecutorKind:     model.ToolExecutorKindMCP,
			EgressHosts:      []string{"mcp.example"},
			ParametersJSON:   []byte(`{"type":"object"}`),
			CredentialRef:    "secrets/tenant/mcp",
			TimeoutMs:        1500,
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(policy.Timeout.CallTimeout).To(Equal(1500 * time.Millisecond))
		Expect(policy.ResponseCap.MaxBytes).To(Equal(int64(32768)))
		Expect(policy.Credential.SecretRef).To(Equal("secrets/tenant/mcp"))
	})
