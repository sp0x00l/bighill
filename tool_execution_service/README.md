...
`tool_execution_service` runs tools that an agent model is allowed to call.
...
`tool_execution_service` owns the safety boundary for tools that act outside inference.
...
`tool_execution_service` owns its own audit database. Inference still records the agent trajectory, but the
...
agent loop asks for a world-acting tool
  -> inference_service calls tool_execution_service over gRPC
  -> tool_execution_service validates tenant + arguments + egress policy
  -> tool_execution_service executes the tool or fails closed
```
...
- the caller org must be in `TOOL_EXECUTION_SERVICE_ALLOWED_ORG_IDS`
- the URL host must be in `TOOL_EXECUTION_SERVICE_HTTP_TOOL_ALLOWED_HOSTS`
...
`tool_catalog_service` is now the source of truth for dynamic tool metadata, tenant grants, and
credential bindings. It publishes catalog events; `tool_execution_service` stores a local projection
and resolves tools from that local state at invocation time. There is no hot-path catalog call.

Projected tools inherit the same boundary controls as static tools: JSON Schema validation, tenant
grant checks, credential reference resolution, egress allowlists, timeout, response cap, classified
failure results, and durable audit.

### Pinned MCP Tools

`tool_execution_service` can also expose one operator-pinned HTTP MCP server. This is deliberately not an MCP
catalog or governance plane. It remains as an operator-configured bridge for a single integration.
Dynamic publish, grants, and credential binding live in `tool_catalog_service`.

The pinned MCP slice works like this:

- startup reads `TOOL_EXECUTION_SERVICE_PINNED_MCP_*` config
...
tool-execution-service config. The Helm chart can mount a Kubernetes secret into an environment variable named
by `TOOL_EXECUTION_SERVICE_PINNED_MCP_CREDENTIAL_REF`.
...
- `TOOL_EXECUTION_SERVICE_GRPC_PORT`: gRPC port, local default `7084`
- `TOOL_EXECUTION_SERVICE_DB_NAME`: audit database name
- `TOOL_EXECUTION_SERVICE_DB_USER`: audit database user
- `TOOL_EXECUTION_SERVICE_DB_PASSWORD`: audit database password
- `TOOL_EXECUTION_SERVICE_DB_MAX_CONNECTIONS`: audit database connection pool limit
- `PGHOST`, `PGPORT`, `PGSSLMODE`: shared Postgres connection settings
- `TOOL_EXECUTION_SERVICE_HEALTHCHECK_PORT`: health port, local default `5065`
- `TOOL_EXECUTION_SERVICE_HEALTHCHECK_DB_LATENCY_THRESHOLD_SECONDS`: health check DB latency threshold
- `TOOL_EXECUTION_SERVICE_HTTP_TOOL_ALLOWED_HOSTS`: comma-separated host allowlist for `http_get`
- `TOOL_EXECUTION_SERVICE_ALLOWED_ORG_IDS`: comma-separated org allowlist
- `TOOL_EXECUTION_SERVICE_HTTP_TOOL_TIMEOUT_MS`: per-request timeout
- `TOOL_EXECUTION_SERVICE_HTTP_TOOL_MAX_RESPONSE_BYTES`: response size cap
- `TOOL_EXECUTION_SERVICE_PINNED_MCP_SERVER_ENDPOINT`: optional HTTP MCP JSON-RPC endpoint
- `TOOL_EXECUTION_SERVICE_PINNED_MCP_SERVER_TRANSPORT`: must be `http` for this slice
- `TOOL_EXECUTION_SERVICE_PINNED_MCP_TOOL_NAMES`: comma-separated declared MCP tools to expose
- `TOOL_EXECUTION_SERVICE_PINNED_MCP_CREDENTIAL_REF`: opaque runtime secret reference for MCP auth
- `TOOL_EXECUTION_SERVICE_PINNED_MCP_ALLOWED_ORG_IDS`: comma-separated org allowlist for pinned MCP tools
...
pkg/infra/repo/db     durable boundary audit and catalog projection repositories
