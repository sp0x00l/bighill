...
CREATE TABLE IF NOT EXISTS bighill_model_registry_db.effective_base_versions (
    effective_base_id text PRIMARY KEY,
    foundation_model_id uuid NOT NULL REFERENCES bighill_model_registry_db.models(model_id),
    ...
    CONSTRAINT effective_base_versions_non_empty_identity_ck CHECK (
        btrim(effective_base_id) <> ''
        AND descriptor_schema_version > 0
        AND btrim(foundation_checksum) <> ''
        AND descriptor <> '{}'::jsonb
    )
);

CREATE INDEX IF NOT EXISTS index_effective_base_versions_foundation_model_id
ON bighill_model_registry_db.effective_base_versions(foundation_model_id, updated_at DESC);

CREATE INDEX IF NOT EXISTS index_effective_base_versions_schema_version
ON bighill_model_registry_db.effective_base_versions(descriptor_schema_version, updated_at DESC);
