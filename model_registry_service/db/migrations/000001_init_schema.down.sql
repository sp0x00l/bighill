DROP TRIGGER IF EXISTS published_inference_endpoints_updated_at ON bighill_model_registry_db.published_inference_endpoints;
DROP INDEX IF EXISTS bighill_model_registry_db.index_published_inference_endpoints_natural_key;
DROP INDEX IF EXISTS bighill_model_registry_db.index_published_inference_endpoints_model_id;
DROP INDEX IF EXISTS bighill_model_registry_db.index_published_inference_endpoints_org_id;
DROP TABLE IF EXISTS bighill_model_registry_db.published_inference_endpoints;
DROP TRIGGER IF EXISTS effective_base_versions_updated_at ON bighill_model_registry_db.effective_base_versions;
DROP INDEX IF EXISTS bighill_model_registry_db.index_effective_base_versions_schema_version;
DROP INDEX IF EXISTS bighill_model_registry_db.index_effective_base_versions_foundation_model_id;
DROP TABLE IF EXISTS bighill_model_registry_db.effective_base_versions;
DROP TRIGGER IF EXISTS models_updated_at ON bighill_model_registry_db.models;
DROP TRIGGER IF EXISTS tenants_updated_at ON bighill_model_registry_db.tenants;
DROP TABLE IF EXISTS bighill_model_registry_db.models;
DROP TABLE IF EXISTS bighill_model_registry_db.tenants;
DROP TYPE IF EXISTS serving_protocol_enum;
DROP TYPE IF EXISTS promotion_decision_enum;
DROP TYPE IF EXISTS model_source_enum;
DROP TYPE IF EXISTS model_kind_enum;
DROP TYPE IF EXISTS model_load_status_enum;
DROP TYPE IF EXISTS model_status_enum;

DROP FUNCTION IF EXISTS updated_at_column();
