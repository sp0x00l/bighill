CREATE TABLE IF NOT EXISTS bighill_model_registry_db.outbox_messages (
...
ON bighill_model_registry_db.outbox_messages(status, next_attempt_at, created_at);

CREATE INDEX IF NOT EXISTS index_outbox_messages_processing
ON bighill_model_registry_db.outbox_messages(status, lease_expires_at, created_at);

CREATE INDEX IF NOT EXISTS index_outbox_messages_resource_key
ON bighill_model_registry_db.outbox_messages(resource_key, created_at);
