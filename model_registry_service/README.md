...
If the champion and candidate metrics are not comparable because the eval dataset, metric suite, or evaluator version changed, the default promotion gate rejects the candidate with a `champion metrics incomparable` reason. The older absolute-floor-only fallback still exists as an explicit policy opt-out and logs a warning when used, but it is not the default path. Deepchecks and Evidently evidence are produced through the same event choreography, but they are binding only when the registry gate policy and `TRAINING_SERVICE_PROMOTION_PROFILE` require them.
