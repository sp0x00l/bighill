...
var _ = Describe("model registry migration policy", func() {
	It("keeps tenant-scoped tables fail-closed", func() {
		...
		Expect(content).To(ContainSubstring("ALTER TABLE bighill_model_registry_db.tenants FORCE ROW LEVEL SECURITY"))
		Expect(content).To(ContainSubstring("NULLIF(current_setting('app.current_user_id', true), '')::uuid = id"))
		for _, table := range []string{"models", "published_inference_endpoints"} {
			Expect(content).To(ContainSubstring("ALTER TABLE bighill_model_registry_db." + table + " FORCE ROW LEVEL SECURITY"))
