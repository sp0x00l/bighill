...
import (
	...
	"tenant_service/pkg/infra/network/messaging"
...
func main() {
	...
	dbConfig.RequireDbName("TENANT_SERVICE_DB_NAME")
	dbConfig.RequireDbUser("TENANT_SERVICE_DB_USER")
	dbConfig.RequireDbPassword("TENANT_SERVICE_DB_PASSWORD")
	dbConfig.RequireDbMaxConnections("TENANT_SERVICE_DB_MAX_CONNECTIONS")
...
func loadConfig() runtimeConfig {
	...
	return runtimeConfig{
		serviceName:             env.MustString("TENANT_SERVICE_NAME"),
		servicePort:             env.MustInt("TENANT_SERVICE_HTTP_PORT"),
		authExpirationInMinutes: env.MustInt("TENANT_SERVICE_AUTH_EXPIRATION_MINUTES"),
		emailValidationTTL:      time.Duration(env.MustInt("TENANT_SERVICE_EMAIL_VALIDATION_TTL_MINUTES")) * time.Minute,
		oauthStateTTL:           time.Duration(env.MustInt("TENANT_SERVICE_OAUTH_STATE_TTL_MINUTES")) * time.Minute,
		googleOAuth: rest.OAuthProviderConfig{
			ClientID:     env.MustString("TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_ID"),
			ClientSecret: env.MustString("TENANT_SERVICE_OAUTH_GOOGLE_CLIENT_SECRET"),
		},
		discordOAuth: rest.OAuthProviderConfig{
			ClientID:     env.MustString("TENANT_SERVICE_OAUTH_DISCORD_CLIENT_ID"),
			ClientSecret: env.MustString("TENANT_SERVICE_OAUTH_DISCORD_CLIENT_SECRET"),
		},
		redisOption: rueidis.ClientOption{
			InitAddress: []string{env.MustString("TENANT_SERVICE_REDIS_ADDRESS")},
			Username:    env.MustString("TENANT_SERVICE_REDIS_USERNAME"),
			Password:    env.MustString("TENANT_SERVICE_REDIS_PASSWORD"),
		},
		messagingConfig: messagingConn.MessengerConfig{
			DlqURL:  env.MustString("TENANT_SERVICE_DLQ"),
			GroupID: env.MustString("TENANT_SERVICE_KAFKA_GROUP_ID"),
			Brokers: brokers,
		},
		outboxBackend: env.MustString("TENANT_SERVICE_OUTBOX"),
		outboxRelayConfig: messagingConn.OutboxRelayConfig{
			PollInterval:   time.Duration(env.MustInt("TENANT_SERVICE_OUTBOX_RELAY_POLL_MS")) * time.Millisecond,
			FailureBackoff: time.Duration(env.MustInt("TENANT_SERVICE_OUTBOX_RELAY_FAILURE_BACKOFF_MS")) * time.Millisecond,
			BatchSize:      int32(env.MustInt("TENANT_SERVICE_OUTBOX_RELAY_BATCH_SIZE")),
		},
		kafkaPublisherTopic: env.MustString("TENANT_SERVICE_KAFKA_PUBLISHER_TOPIC"),
		useStagingTestToken: env.WithDefaultBool("TENANT_SERVICE_USE_STAGING_TEST_EMAIL_TOKEN", env.IsStaging()),
		huggingFaceTokenKey: env.MustString("TENANT_SERVICE_HUGGINGFACE_TOKEN_ENCRYPTION_KEY"),
		lifecycleConfig: lifecycle.Config{
			ReadinessTimeout: secondsFromEnv("TENANT_SERVICE_LIFECYCLE_READINESS_TIMEOUT_SECONDS", "30"),
			DrainTimeout:     secondsFromEnv("TENANT_SERVICE_LIFECYCLE_DRAIN_TIMEOUT_SECONDS", "30"),
			CloseTimeout:     secondsFromEnv("TENANT_SERVICE_LIFECYCLE_CLOSE_TIMEOUT_SECONDS", "10"),
		},
		healthCheckConfig: healthcheck.HealthCheckConfig{
			CpuThresholdPercentage:           env.MustInt("TENANT_SERVICE_HEALTHCHECK_CPU_THRESHOLD_PERCENT"),
			MemFreeThresholdPercentage:       env.MustInt("TENANT_SERVICE_HEALTHCHECK_FREE_MEM_THRESHOLD_PERCENT"),
			HealthCheckPort:                  env.MustInt("TENANT_SERVICE_HEALTHCHECK_PORT"),
			ServiceLatencyThresholdSec:       time.Duration(env.MustInt("TENANT_SERVICE_HEALTHCHECK_SERVICE_LATENCY_THRESHOLD_SECONDS")) * time.Second,
			DbLatencyThresholdSec:            time.Duration(env.MustInt("TENANT_SERVICE_HEALTHCHECK_DB_LATENCY_THRESHOLD_SECONDS")) * time.Second,
			MessageBrokerLatencyThresholdSec: time.Duration(env.MustInt("TENANT_SERVICE_HEALTHCHECK_KAFKA_LATENCY_THRESHOLD_SECONDS")) * time.Second,
