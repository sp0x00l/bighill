...
`tenant_service` owns users, profiles, authentication, sessions, and profile-domain events. It is the identity and tenant boundary for the rest of the platform.
...
Configuration is provided through `TENANT_SERVICE_` env vars from `scripts/config.sh`, Helm, docker-compose, and launch settings. The service runs as part of the root local-dev/test workflow.
