...
var _ = Describe("Profile server entry points", Ordered, func() {
	...
	Describe("bighill profile end-points", func() {
		BeforeAll(func() {
			...
			dbConfig.WithDbUser("TENANT_SERVICE_DB_USER", "bighill_tenant_db_user")
			dbConfig.WithDbPassword("TENANT_SERVICE_DB_PASSWORD", "")
			dbConfig.WithDbMaxConnections("TENANT_SERVICE_DB_MAX_CONNECTIONS", "60")
		...
		BeforeEach(func() {
			...
			redisAddr := env.WithDefaultString("TENANT_SERVICE_REDIS_ADDRESS", "localhost:6379")
			redisUser := env.WithDefaultString("TENANT_SERVICE_REDIS_USERNAME", "")
			redisPassword := env.WithDefaultString("TENANT_SERVICE_REDIS_PASSWORD", "")
			...
			Expect(err).ShouldNot(HaveOccurred())
			authExpirationInMinutes := env.WithDefaultInt("TENANT_SERVICE_AUTH_EXPIRATION_MINUTES", "15")
			profilesUseCase = usecase.NewProfilesUseCase(
				...
				usecase.ProfilesUseCaseConfig{
					...
					UseStagingTestToken:     env.WithDefaultBool("TENANT_SERVICE_USE_STAGING_TEST_EMAIL_TOKEN", false),
				},
