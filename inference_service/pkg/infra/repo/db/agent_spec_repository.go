...
func (r *AgentSpecRepository) UpsertAgentSpec(ctx context.Context, spec *model.AgentSpec) (*model.AgentSpec, error) {
	...
	query := `INSERT INTO ` + r.Name + `.agent_specs (
		...
		validation_report, model_id, effective_base_id, tool_bindings, retrieval_config,
		budgets, stop_conditions, guardrails, runtime_mode, status
	) VALUES (
		@org_id, @agent_lineage, @system_prompt, @source_yaml, @canonical_json::jsonb, @schema_version, @content_hash,
		@validation_report, @model_id, @effective_base_id, @tool_bindings::jsonb, @retrieval_config::jsonb,
		@budgets::jsonb, @stop_conditions::jsonb, @guardrails::jsonb, @runtime_mode::agent_runtime_mode_enum, @status::agent_spec_status_enum
	)
...
func agentSpecArgs(spec *model.AgentSpec) pgx.NamedArgs {
	...
	return pgx.NamedArgs{
		...
		"effective_base_id": nullableUUID(spec.EffectiveBaseID),
		...
		"runtime_mode":      spec.RuntimeMode.String(),
...
func scanAgentSpec(row pgx.Row) (*model.AgentSpec, error) {
	...
	var agentSpecID, orgID, canonicalJSON, modelID, effectiveBaseID, toolBindings, retrievalConfig, budgets, stopConditions, guardrails, runtimeMode, status string
	...
	parsedEffectiveBaseID, err := uuid.Parse(effectiveBaseID)
	if err != nil {
		return nil, fmt.Errorf("parse agent spec effective base id: %w", err)
	}
	record.EffectiveBaseID = parsedEffectiveBaseID
	...
	parsedRuntimeMode, err := model.ToAgentRuntimeMode(runtimeMode)
	if err != nil {
		return nil, fmt.Errorf("parse agent spec runtime mode: %w", err)
	}
	parsedStatus, err := model.ToAgentSpecStatus(status)
	if err != nil {
		return nil, fmt.Errorf("parse agent spec status: %w", err)
	}
	record.RuntimeMode = parsedRuntimeMode
...
func (r *CapabilityReportRepository) RecordCapabilityReport(ctx context.Context, report *model.CapabilityReport) (*model.CapabilityReport, error) {
	...
	conflictClause := `ON CONFLICT (org_id, model_id) WHERE model_id IS NOT NULL AND effective_base_id IS NULL DO UPDATE SET
		supports_chat = EXCLUDED.supports_chat,
		supports_tool_calls = EXCLUDED.supports_tool_calls,
		supports_json_schema_output = EXCLUDED.supports_json_schema_output,
		supports_system_prompt = EXCLUDED.supports_system_prompt,
		context_window_tokens = EXCLUDED.context_window_tokens,
		max_output_tokens = EXCLUDED.max_output_tokens`
	if report.EffectiveBaseID != uuid.Nil {
		conflictClause = `ON CONFLICT (org_id, effective_base_id) WHERE effective_base_id IS NOT NULL DO UPDATE SET
			supports_chat = EXCLUDED.supports_chat,
			supports_tool_calls = EXCLUDED.supports_tool_calls,
			supports_json_schema_output = EXCLUDED.supports_json_schema_output,
			supports_system_prompt = EXCLUDED.supports_system_prompt,
	...
	query := `INSERT INTO ` + r.Name + `.capability_reports (
		org_id, effective_base_id, model_id, supports_chat, supports_tool_calls,
		supports_json_schema_output, supports_system_prompt, context_window_tokens, max_output_tokens
	) VALUES (
		@org_id, @effective_base_id, @model_id, @supports_chat, @supports_tool_calls,
		@supports_json_schema_output, @supports_system_prompt, @context_window_tokens, @max_output_tokens
	)
	...
	RETURNING capability_report_id::text,
		org_id::text,
		COALESCE(effective_base_id::text, ''),
...
func (r *CapabilityReportRepository) ReadCapabilityReportForModel(ctx context.Context, orgID uuid.UUID, modelID uuid.UUID, effectiveBaseID uuid.UUID) (*model.CapabilityReport, error) {
	...
	query := `SELECT capability_report_id::text,
		...
		WHERE org_id = @org_id AND ((
				@effective_base_id::uuid IS NOT NULL AND effective_base_id = @effective_base_id
			) OR (
				@effective_base_id::uuid IS NULL AND model_id = @model_id
			))
		ORDER BY created_at DESC
		LIMIT 1`
	record, err := scanCapabilityReport(r.Pool.QueryRow(ctx, query, pgx.NamedArgs{
		"effective_base_id": nullableUUID(effectiveBaseID),
...
func scanCapabilityReport(row pgx.Row) (*model.CapabilityReport, error) {
	...
	var capabilityReportID, orgID, effectiveBaseID, modelID string
	...
	if err := row.Scan(
		...
		&record.SupportsJSONSchemaOutput,
	...
	if effectiveBaseID != "" {
		parsedEffectiveBaseID, err := uuid.Parse(effectiveBaseID)
		if err != nil {
			return nil, fmt.Errorf("parse capability report effective base id: %w", err)
		}
		record.EffectiveBaseID = parsedEffectiveBaseID
	}
	if modelID != "" {
