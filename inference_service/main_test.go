...
var _ = Describe("readInferenceConfig", func() {
	BeforeEach(func() {
		...
		Expect(os.Unsetenv("INFERENCE_PREFERENCE_DATASET_EXPORT_ENABLED")).To(Succeed())
		Expect(os.Unsetenv("INFERENCE_PREFERENCE_DATASET_URI_TEMPLATE")).To(Succeed())
		Expect(os.Unsetenv("INFERENCE_PREFERENCE_DATASET_MIN_EXAMPLES")).To(Succeed())
		Expect(os.Unsetenv("INFERENCE_PREFERENCE_DATASET_LIMIT")).To(Succeed())
		Expect(os.Unsetenv("INFERENCE_PREFERENCE_DATASET_BUCKET_REGION")).To(Succeed())
		Expect(os.Unsetenv("INFERENCE_PREFERENCE_DATASET_UPLOAD_PART_SIZE_MB")).To(Succeed())
	...
	It("uses local defaults", func() {
		...
		Expect(cfg.PreferenceDataset.URITemplate).To(Equal("s3://local-dev-bucket/preferences/{dataset_id}/preference_dataset.jsonl"))
